...
(use-package hyperspec
  :config
  (progn
    (let ((hyperspec-root (ef "/pub/dok/komputiko/lingvoj/lispo/hyperspec/")))
...
(use-package slime
  ...
  :config
  (progn
    (load (ef "~/quicklisp/slime-helper.el"))
    ...
    (defslime-repl-shortcut nil  ("restart-inferior-lisp" "r")
...
(use-package xmodmap-mode
  :config
  (push '("\\.Xmodmap\\'" . xmodmap-mode) auto-mode-alist))
...
(use-package nasm-mode
  :ensure t
  :config
  (push '("\\.nasm\\'" . nasm-mode) auto-mode-alist))
...
(use-package csv-mode
  :ensure t
  :config
  (progn (setq csv-separators-char '(44 9 59)
               csv-separators '("," "	" ";")
...
(use-package ethan-wspace
  :ensure t
  :config
  (progn (global-ethan-wspace-mode 1)
...
;;; framerd
(use-package fdconsole
  :config
  (progn
    (setq auto-mode-alist
          (append '(("\\.fdx" . scheme-mode)
                    ("\\.fdz" . scheme-mode)
                    ("\\.fdcgi" . scheme-mode))
                  auto-mode-alist))))
...
(use-package racket-mode
  ...
  :config
  (progn
    (setq auto-mode-alist (append '(("\\.rkt" . racket-mode)) auto-mode-alist))
...
(use-package nxml-mode
  ...
  (progn
    (setq auto-mode-alist
          (append '(("\\.aiml" . nxml-mode))
                  auto-mode-alist))
    (add-hook 'nxml-mode-hook 'turn-on-smartparens-strict-mode)))
